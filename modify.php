...
#TODO:
#Must first introduce drop down menu 
#Select user from it to modify
#Populate form with that user's information
# Resubmit form to alter user info
...
#Form starts here!
echo '<br />
	<form style="position:relative; left:20%;" method="POST" action="modify.php?action=form_create">
	Modify who? <br />
...
#generates the form required for record modification
function form_create()
{	
global $db;

$customer_id = $_POST['customer'];
$query = 'SELECT * FROM shark_loan_form WHERE customer_id=' . $customer_id;

#Last confirmed working point, likely the query method is crashing the PHP script.

if(!($result=$db->query($query)))
{
	echo 'Query failed. Disconnecting...';
	disconnect();	
}
else
{
#Grab desired row here.
$row = $result->fetch_assoc();

#Enable modification by populating form w/ current data from query
echo '<form id="calculator_form" action="modify.php?action=confirm" method="POST">
    	<fieldset>
        	<legend>Personal Information:</legend>
    	First Name:<input name="f_name_in" maxlength="16" title="Letters only. Less than 16 characters." value="' . $row['First_name'] . '" />
        Last Name: <input name="l_name_in" maxlength="16" title="Letters only. Less than 16 characters." value="' . $row['Last_name'] . '"/>
        <br />
        Address 1: <input name="address_1_in" maxlength="40" title="Primary address information." value="' . $row['Address_1'] . '"/>
        <br />
        Address 2: <input name="address_2_in" maxlength="40" title="Apartment number or other secondary address info." placeholder="Not required." value="' . $row['Address_2'] . '"/>
        <br />
		City: <input name="city_in" maxlength="25" value="' . $row['City'] . '"/>
        ...
        Zip Code: <input name="zip_code" size="5" maxlength="5" title="First 5 digits of your zip code." value="' . $row['Zip'] . '"/>
        <br />
        Telephone Number: 
        <input name="phone_1" size="3" maxlength="3" title="Area code goes here." value="'.$row['Phone_1'].'"/> -
        <input name="phone_2" size="3" maxlength="3" title="Next 3 digits go here." value="'.$row['Phone_2'].'"/> -        
        <input name="phone_3" size="4" maxlength="4" title="Last 4 digits go here." value="'.$row['Phone_3'].'"/>
                <hr id="form_hr" />   
        Social Security Number:
        <input name="social_1" size="3" maxlength="3" title="DONT PUT REAL SS NUMBER IN HERE" value="'.$row['Social_1'].'"/> - 
        <input name="social_2" size="2" maxlength="2" title="DONT PUT REAL SS NUMBER IN HERE" value="'.$row['Social_2'].'"/> - 
        <input name="social_3" size="4" maxlength="4" title="DONT PUT REAL SS NUMBER IN HERE" value="'.$row['Social_3'].'"/>
        <br />
        Amount to Borrow: <input name="loan_amount" size="10" maxlength="20" title="Input amount you wish to borrow." value="'.$row['Loan_amount'].'"/> USD
        <br />
        Length of Loan <input name="loan_length" size="3" maxlength="3" title="Input amount you wish to borrow." value="'.$row['Loan_length'].'"/> (months)
        <br />
        Points:
        <select name="point_charge">
			<option selected value="'.$row['Point_charge'].'">'.$row['Point_charge'].'</option>
        ...
        <br />
        <input type="submit" value="Modify Records" />
    	</fieldset>
    </form>';
}
}
...
{	
	...
	echo '<form action="modify.php?action=insert" method="POST">
...
#Set the modification query
$query='UPDATE shark_loan_form
...
if (!($result=$db->query($query)))
{
	...
	echo '<br /><br />
<a href="modify.php?action=form_create">Modify another record</a>';
	disconnect();
} 
